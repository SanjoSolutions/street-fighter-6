...
for character in characters:
    ...
    if os.path.isfile(moves_path):
        ...
        with open(character_directory + '/connecting_moves.csv',
            ...
            selected_combos = [combo_with_metadata for combo_with_metadata in combos_with_metadata if 'Crossfire Somersault' not in combo_with_metadata['combo'] and 'Sonic Hurricane' not in combo_with_metadata['combo'] and 'Solid Puncher' not in combo_with_metadata['combo'] and not any(('Sonic Boom', 'Burning Straight') == combo_with_metadata['combo'][index:index+2] or ('OD Sonic Boom', 'Heavy Somersault Kick') == combo_with_metadata['combo'][index:index+2] for index in range(0, len(combo_with_metadata['combo'])))]
